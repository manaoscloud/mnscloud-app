...
export class SupportAttendancePage implements AfterViewInit {
    ...
    private updateFilterOptions() {
        ...
        this.sectorOptions = this.uniqueValues(this.items.map(item => item.Sector).filter(Boolean) as string[]);
        this.agentOptions = this.uniqueValues(this.items.map(item => item.AssignedTo).filter(Boolean) as string[]);
    }
    ...
    private applyFilters() {
        ...
        const filtered = this.items.filter(item => {
            ...
            if (search) {
                const haystack = [name, ch, sec, ag, item.Identifier].filter(Boolean).join(' ').toLowerCase();
    ...
    private resolveStatus(item: AttendanceItem): AttendanceStatus {
        ...
        if (raw.includes('service') || raw.includes('attendance') || raw.includes('serving') || raw.includes('active')) {
            return 'serving';
        }
