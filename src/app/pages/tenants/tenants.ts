...
export class SettingsTenantsPage {
  ...
  private async loadAccessList() {
    ...
    try {
      ...
      const selfList = resSelf?.data?.access ?? [];
      ...
      const currentRole =
        selfList.find((item: any) => item.EnvironmentUUID === currentEnv)?.Role ?? null;
      ...
      if (currentEnv && ['OWNER', 'ADMIN'].includes(currentRole)) {
        try {
          ...
          const envList = resEnv?.data?.members ?? [];
      ...
      const message = err?.error?.message || err?.message || 'Failed to load access list.';
  ...
  private async loadInvites() {
    ...
    try {
      const res = await this.service.listInvites();
      ...
      const message = err?.error?.message || err?.message || 'Failed to load invitations.';
  ...
  async sendInvite() {
    ...
    try {
      ...
      const message = res?.message || res?.data?.message || 'Invitation sent successfully.';
      ...
      const msg =
        err?.error?.message || err?.error?.error || err?.message || 'Failed to send invitation.';
