...
export class IspFixedIpv6Page implements AfterViewInit, OnDestroy {
  ...
  async loadItems() {
    ...
    try {
      ...
      this.error.set(this.extractErrorMessage(err, 'Failed to load fixed IPv6 entries.'));
    } finally {
  ...
  async delete(item: FixedIpv6Item) {
    ...
    try {
      ...
      this.dataSource.data = this.dataSource.data.filter((row) => row.If6UUID !== item.If6UUID);
    } catch (err: any) {
