...
export class RealtimeMediaPage {
  ...
  installCommand(): string {
    ...
    const args = [
      ...
      '--control-ip',
      this.shellQuote(String(source?.['RmsControlIP'] || token['controlIP'] || '0.0.0.0')),
      '--control-port',
  ...
  installCommandDetails(): Array<{ label: string; value: unknown; monospace?: boolean }> {
    ...
    return [
      ...
      { label: 'Node UUID', value: token?.['nodeUUID'], monospace: true },
      {
        ...
        value: `${token?.['controlIP'] ?? '0.0.0.0'}:${token?.['controlPort'] ?? 2223}`,
        monospace: true,
      },
  ...
  cell(row: MediaRecord, column: string): string {
    const map: Record<string, any> = {
      ...
      control: `${row['RmsControlIP'] || '0.0.0.0'}:${row['RmsControlPort'] ?? 2223}`,
  ...
  private formModelFromRow(row: MediaRecord): MediaFormModel {
    ...
    return {
      ...
      controlIP: row['RmsControlIP'] ?? '0.0.0.0',
