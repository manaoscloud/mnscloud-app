...
@Component({
  ...
  standalone: true,
  imports: [MatChipsModule, TranslocoPipe, MnsDateTimePipe, MatSelectModule, MatFormFieldModule, MatPaginatorModule],
  template: `
    <section class="dashboard-panel">
      <div class="panel-header">
        ...
        @if (sortOptions().length) {
          <mat-form-field appearance="outline">
            ...
            <mat-select [value]="sortActive() + ':' + sortDirection()" (selectionChange)="changeSort($event.value)">
              @for (option of sortOptions(); track option.key) {
                <mat-option [value]="option.key + ':asc'">{{ option.label | transloco }} · {{ 'Ascending' | transloco }}</mat-option>
                <mat-option [value]="option.key + ':desc'">{{ option.label | transloco }} · {{ 'Descending' | transloco }}</mat-option>
              }
            </mat-select>
          </mat-form-field>
        }
      ...
      @if (total() !== null) {
        <mat-paginator class="mobile-paginator" [length]="total() ?? 0" [pageIndex]="pageIndex()"
          [pageSize]="pageSize()" [pageSizeOptions]="[5, 10, 25]" (page)="page.emit($event)" showFirstLastButtons />
      }
      <ng-content />
    </section>
  `,
