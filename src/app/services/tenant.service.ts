...
interface UserAccessApiResponse {
    ...
    message: string;
    data?: {
        getUserAccess?: TenantAccess[];
        accessList?: TenantAccess[];
    };
}
...
export class TenantService {
    ...
    async loadTenants(): Promise<void> {
        ...
        try {
            ...
            if (saved) {
                const found = list.find((t: TenantAccess) =>
                    t.EnvironmentUUID === saved
                );
            ...
            const defaultTenant =
                list.find((t: TenantAccess) => Number(t.IsDefault ?? 0) === 1) ?? null;
